...
**3. Policy Scanner Engine** (`parse_policy_csv` → `scan_bidirectional` / `recommend_split`)
Reads a SecureTrack-style policy export. Important format quirks: the **first 3 lines are metadata and skipped**; line 4 is the header. Columns are located by header name with a **fixed-position fallback** (`_col_index`), because the export has stable column letters — Source = col R (index 17), Destination = col U (index 20). `scan_bidirectional` flags rules where Source == Destination (the bi-directional smell). `recommend_split` proposes least-permissive uni-directional replacements by clustering objects that share a /16 or a common leading name token (`_cluster_objects`, `_objects_related`).
...
**Routing layer** (bottom of file): `/` serves a toolbox landing page, `/analyzer` and `/scanner` serve the two tool pages. POST endpoints (`/analyze`, `/download`, `/download_checkpoint`, `/scan`, `/recommend`, `/recommend_download`, `/scan_download`) re-parse the uploaded file on every call — there is no server-side session or caching, so each download endpoint repeats the full parse+analyze pipeline. Most endpoints wrap their body in a broad `try/except` that returns `{"error": str(e)}`.
